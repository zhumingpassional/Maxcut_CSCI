...
class Evaluator:
    def __init__(self, save_dir: str, num_nodes: int, x: TEN, v: float):
        ...
        self.recorder1 = []
        self.recorder2 = []
        self.encoder_base64 = EncoderBase64(encode_len=num_nodes)
    ...
    def record2(self, i: float, v: float, x: TEN):
        ...
        self.recorder2.append((i, v, exec_time))
    ...
    def logging_print(self, x: TEN, v: float, show_str: str = '', if_show_x: bool = False):
        ...
        print(f"|{i:6} {used_time:4} sec  v {v:6.0f} < {self.best_v:6.0f}  {show_str}  {x_str}")
...
class TrainingLogger:
    ...
    def add_and_print(self, repeat_id, buffer_id, objective_item):
        ...
        print(f"| {repeat_id:2} {buffer_id:2}  {exec_time:4} sec | obj {objective_item:9.4f}")

    def save_as_csv_and_plot(self, path: str = './recorder.csv'):
        df = pd.DataFrame(self.recorder, columns=['repeat_id', 'buffer_id', 'exec_time', 'objective'])
        df.to_csv(path, index=False)

    @staticmethod
    def plot_training_recorder(path, ignore_n: int = 8):
        # data_dir = f"./recorder"
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"{path}.jpg", dpi=200)
